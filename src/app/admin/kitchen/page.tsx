...
import KDSAutoRefresh from "@/components/kitchen/KDSAutoRefresh";
    ...
    <div className="min-h-screen bg-black p-4 md:p-12 text-white selection:bg-primary selection:text-black">
      ...
      <MotionDiv className="max-w-7xl mx-auto space-y-10">
        ...
        <MotionItem className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-6 border-b border-white/5">
          ...
          <div className="flex flex-col xs:flex-row items-center justify-between sm:justify-end gap-4 sm:gap-6 w-full sm:w-auto">
            <div className="text-center sm:text-right">
              <p className="text-2xl sm:text-3xl font-black text-white leading-none">
                {pendingCount + inProgressCount}
              </p>
              <p className="text-[9px] sm:text-[10px] uppercase font-bold text-neutral-500 tracking-wider mt-1">
                Pedidos Activos
              </p>
            </div>
            <div className="hidden xs:block h-10 sm:h-12 w-px bg-white/10" />
            <div className="flex items-center gap-2 sm:gap-3 bg-zinc-900/50 border border-white/5 rounded-full pl-3 pr-4 sm:pl-4 sm:pr-6 py-2 w-full xs:w-auto justify-center xs:justify-start">
              <div
                className={`h-2 w-2 sm:h-3 sm:w-3 rounded-full shrink-0 ${estimatedWaitTime > 20 ? "bg-red-500 animate-pulse" : "bg-green-500"}`}
              />
              <div>
                <p className="text-[8px] sm:text-[10px] uppercase font-bold text-neutral-500 leading-none mb-0.5">
                  Demora Est.
                </p>
                  ...
                  <Card
                    className={`glass-card transition-all group ${
                      order.minutesWaiting > 20
                        ? "border-red-500/50 bg-red-500/5 shadow-[0_0_30px_-10px_rgba(239,68,68,0.3)]"
                        : order.minutesWaiting > 10
                          ? "border-orange-500/50 bg-orange-500/5 shadow-[0_0_30px_-10px_rgba(249,115,22,0.3)]"
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              {cookingOrders.map((order) => (
                ...
                >
                  <Card
                    className={`glass-card transition-all group relative overflow-hidden ${
                      order.minutesWaiting > 20
                        ? "border-red-500/50 bg-red-500/5 shadow-[0_0_30px_-10px_rgba(239,68,68,0.3)]"
                        : order.minutesWaiting > 15
                          ? "border-orange-500/50 bg-orange-500/5 shadow-[0_0_30px_-10px_rgba(249,115,22,0.3)]"
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <span
                          className={`text-xs font-bold font-mono px-2 py-1 rounded border ${
                            order.minutesWaiting > 20
                              ? "bg-red-500 text-white border-red-500/50 animate-pulse"
                              : order.minutesWaiting > 15
                                ? "bg-orange-500 text-black border-orange-500/50"
                    ...
                    <CardContent className="p-5 space-y-4">
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex flex-col items-end gap-1">
                          ...
                          {order.minutesWaiting < 5 ? (
                            <div className="flex items-center gap-1 bg-yellow-500/20 text-yellow-500 px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-tighter border border-yellow-500/20">
                              <Trophy className="h-2.5 w-2.5" /> Oro
                            </div>
                          ) : order.minutesWaiting < 10 ? (
                            <div className="flex items-center gap-1 bg-zinc-400/20 text-zinc-400 px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-tighter border border-zinc-400/20">
                              <Trophy className="h-2.5 w-2.5" /> Plata
                            </div>
                          ) : order.minutesWaiting < 15 ? (
                            <div className="flex items-center gap-1 bg-orange-700/20 text-orange-700 px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-tighter border border-orange-700/20">
                              <Trophy className="h-2.5 w-2.5" /> Bronce
                            </div>
                          ) : null}
                        </div>
                      </div>
